...
export const getLogin = async (req, res) => {
  try {
    ...
    const passwordToFind = req.body.email
    ...
    if (usuarios.length === 0) {
      return res
        .status(400)
        .json({ error: "Erro ao processar os dados do usuário." })
    }

    return res.status(200).json([{ email: emailToFind, token: token }])
  } catch (error) {
    console.error("Erro ao buscar o usuário:", error)
    return res
      .status(500)
      .json({ message: "Erro ao processar os dados do usuário." })
  }
}
